 ...
 * @author EinsiTang
 */
const {matrix, range} = require('./tools')
...
const simFill = (source, rows, cols, zones, index, numPool) => {
...
const generator = (fillRules) => {
    ...
    // 基准数据填充
    let basicSource = range(81, () => -1)
    basicIndexes.forEach((index) => {
        simFill(basicSource, rows, cols, zones, index, numPool)

    })

    // 根据基准数据计算出完整数独
    let sudoku1 = new Sudoku(basicSource)
...
module.exports = (level = 0) => {
    ...
    if (!levelRules) {
        throw new Error(`请输入正确的level 0 - ${RULE_LEVEL.length}`)
    }
